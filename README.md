...
4.  The buffer will open in read-only mode. To make changes:
    ...
    e.  To tag the role directly, press `C-c C-t` and enter tags as `key=value` pairs separated by `, `.
...
| `:tags`         | Optional. Must be a quoted string of `key=value` pairs separated by `, ` (comma + space). | `"owner=hello there, name=Will"` |
